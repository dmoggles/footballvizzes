...
class ScatterPlot:
    ...
    def _draw_title(cls, ax: Axes, title: str, subtitle: str):
        ax.text(0.5, 0.5, title, ha="center", va="center", size=cls.TITLE_SIZE, fontproperties=font_bold.prop)
        if subtitle:
            ax.text(
                0.5, -0.2, subtitle, ha="center", va="center", size=cls.SUBTITLE_SIZE, fontproperties=font_italic.prop
            )
    ...
    def _add_endnote_annotations(cls, ax: Axes, explanatory_comment: str):
        if explanatory_comment:
            ax.text(0.01, 1.1, explanatory_comment, size=10, ha="left", va="bottom", fontproperties=font_italic.prop)
        ax.text(0.99, 0.5, "@mclachbot\nData:FbRef", size=10, va="center", ha="center", fontproperties=font_normal.prop)

    @classmethod
    def _draw_endnote(cls, ax: Axes, competitions: Sequence[str], explanatory_comment: str):
    ...
    def _setup_data(cls, df, x_param, y_param):
        model1 = IsolationForest(contamination=cls.COLOR_MARKING_THRESHOLD, random_state=0)
        model2 = IsolationForest(contamination=cls.NAME_ANNOTATION_THRESHOLD, random_state=0)

        df["inner_ring"] = model1.fit_predict(df[[x_param, y_param]])
        df["annotate"] = model2.fit_predict(df[[x_param, y_param]])

    @classmethod
    def _setup_scatter_axes(cls, ax: Axes, df: pd.DataFrame, x_param: str, y_param: str):
    ...
    @classmethod
    def _scatter_inner_points(cls, ax: Axes, df: pd.DataFrame, x_param: str, y_param: str):
        inner_df = df.loc[df["inner_ring"] == 1]
        ax.scatter(inner_df[x_param], inner_df[y_param], c=cls.INNER_DATA_COLOUR, alpha=0.5, zorder=2)

    @classmethod
    def _scatter_league_marked_points(cls, ax: Axes, df: pd.DataFrame, x_param: str, y_param: str):
        ...
        for league in leagues:
            ...
            ax.scatter(df_[x_param], df_[y_param], c=color, edgecolor=border, s=100, zorder=3)

    @classmethod
    def _annotate_player_names(cls, ax: Axes, df: pd.DataFrame, x_param: str, y_param: str) -> list:
        annotate_df = df.loc[df["annotate"] == -1]
        annotate_df["name"] = annotate_df[fb.PLAYER.N].apply(lambda x: cls._player_name(x))
    ...
    def _draw_halves(cls, ax: Axes, df: pd.DataFrame, x_param: str, y_param: str):
        ...
        ax.axvline([mean_x], ymin=0, ymax=1, linestyle="--", lw=1, color=cls.MEAN_LINE_COLOR, zorder=1)
        ax.axhline([mean_y], xmin=0, xmax=1, linestyle="--", lw=1, color=cls.MEAN_LINE_COLOR, zorder=1)
        ax.fill(
    ...
    def draw(
        ...
        cls._draw_endnote(axes[cls.ENDNOTE_IDX], data[fb.COMPETITION.N].unique(), explanatory_comment)

        cls._draw_scatter(axes[cls.SCATTER_PLOT_IDX], data, x_column, y_column, draw_halves, half_annotations)
