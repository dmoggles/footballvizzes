...
def make_grid(
    ...
    if (nrows > 1) and (ncols > 1):
        ...
        figwidth = (
            grid_height * figheight / grid_width * (1 - space) * pitch.ax_aspect / nrows
        )
        individual_space_height = grid_height * space / (nrows - 1)
        ...
        figwidth = (
            grid_height * figheight / grid_width * (space + pitch.ax_aspect * ncols)
        )
        individual_space_height = 0
        individual_space_width = (
            grid_height * space * figheight / figwidth / (ncols - 1)
        )
    ...
    individual_pitch_width = (
        individual_pitch_height * pitch.ax_aspect * figheight / figwidth
    )
    ...
    left_pad = (
        np.abs(pitch.visible_pitch - pitch.extent)[0]
        / np.abs(pitch.extent[1] - pitch.extent[0])
    ) * individual_pitch_width
    right_pad = (
        np.abs(pitch.visible_pitch - pitch.extent)[1]
        / np.abs(pitch.extent[1] - pitch.extent[0])
    ) * individual_pitch_width
